...
describe("createParams", () => {
  ...
  it("should log an error if the provided element is null", () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "createParams: Provided element is null or undefined.",
    );
...
describe("createNoscriptFallback", () => {
  it("should create a noscript element with a fallback image", () => {
    ...
    expect(img?.getAttribute("alt")).toBe(
      "Fallback image for Tableau dashboard",
    );
  });
});
...
describe("loadScript", () => {
  ...
  it("should log an error if the script fails to load", () => {
    ...
    const script = document.querySelector(
      'script[src="https://invalid-url.com/script.js"]',
    );
    expect(script).toBeDefined();
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "Failed to load script: https://invalid-url.com/script.js",
    );
    consoleErrorSpy.mockRestore();
  });
});
